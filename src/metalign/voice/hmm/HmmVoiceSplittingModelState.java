...
/**
 * A <code>State</code> contains a List of the
 ...
 */
public class HmmVoiceSplittingModelState extends VoiceSplittingModelState {
	...
	public HmmVoiceSplittingModelState(HmmVoiceSplittingModelParameters params) {
		this(0, new ArrayList<Voice>(), params);
	}
	...
	@Override
	public TreeSet<HmmVoiceSplittingModelState> close() {
		TreeSet<HmmVoiceSplittingModelState> newState = new TreeSet<HmmVoiceSplittingModelState>();
	...
	public boolean isDuplicateOf(VoiceSplittingModelState state) {
		if (!(state instanceof HmmVoiceSplittingModelState)) {
			return false;
		}
		
		HmmVoiceSplittingModelState hmm = (HmmVoiceSplittingModelState) state;
		
		if (hmm.voices.size() != voices.size()) {
			return false;
		}
		
		for (int i = 0; i < voices.size(); i++) {
			if (hmm.voices.get(i) != voices.get(i)) {
				return false;
			}
		}
	...
	public int compareTo(MidiModelState other) {
		if (!(other instanceof HmmVoiceSplittingModelState)) {
			return -1;
		}
		
		HmmVoiceSplittingModelState o = (HmmVoiceSplittingModelState) other;

 ...
 */
public class MidiTimeTracker extends TimeTracker {
	/**
    ...
    public List<Beat> getTatums() {
    	...
    	if (tatumNum == 0) {
    		beats.add(new Beat(measureNum, beatNum, subBeatNum, tatumNum, getTimeAtTick(tick)));
    	}
	...
	public TimeSignature getTimeSignatureAtTime(long time) {
		return getNodeAtTime(time).getTimeSignature();
	}
}
 ...
 * the {@link metalign.utils.MidiNote}s present in any given song. 
 * 
...
public class NoteListGenerator implements NoteEventParser {
	...
	public void noteOff(int key, long tick, int channel) throws InvalidMidiDataException {
		...
		//		"key=" + key + ", tick=" + tick + " voice=" + channel);
	}
	 ...
	 * which should be fed into the {@link metalign.generic.MidiModel#handleIncoming(List)}
	 * method.
	 * 
	 * @return A List of incoming note lists, where each individual list contains all of the notes
	 * which onset at a particular time, sorted by offset time, and the list of listsis itself sorted
	 * by onset time.
	 */
	public List<List<MidiNote>> getIncomingLists() {
		...
		while (i < noteList.size()) {
			...
			// Sort notes with identical onsets by offset time
			Collections.sort(incoming, new Comparator<MidiNote>() {
				@Override
				public int compare(MidiNote o1, MidiNote o2) {
					int result = Long.compare(o1.getOnsetTime(), o2.getOnsetTime());
					if (result != 0) {
						return result;
					}
					
					return Long.compare(o1.getOffsetTime(), o2.getOffsetTime());
				}
			});
	...
	public String toString() {
		return completedNotes.toString();
	}
}
